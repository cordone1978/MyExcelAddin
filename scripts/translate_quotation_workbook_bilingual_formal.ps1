...
$outputPath = 'C:\Users\zhuhuihua\Desktop\OMYA DCM1500-FJ Coating Modification System Project_20260205-V01_Bilingual_Formal.xlsx'
$baseScriptPath = 'E:\OfficeAddinProjects\quotationaddin\scripts\translate_quotation_workbook.ps1'
...
$scriptText = Get-Content -Raw -Encoding UTF8 $baseScriptPath
...
if (-not $translationsBlock.Success) {
  throw "Failed to parse translation map from $baseScriptPath"
}

$englishTranslations = Invoke-Expression ('@{' + $translationsBlock.Groups['body'].Value + "`n}")

$formalOverrides = @{
  '报价日期：2026年01月30日' = '报价日期 / Quotation Date: 30 January 2026'
  '报价日期：2026年02月05日' = '报价日期 / Quotation Date: 05 February 2026'
  '付款方式（Payment）：30%+55%+10%+5%' = '付款方式 / Terms of Payment: 30% + 55% + 10% + 5%'
  ...
  '交货时间（Time of Delivery)：' = '交货时间 / Delivery Time:'
  '联系人（Attn.)：耿总' = '联系人 / Contact: Mr. Geng'
  ...
  '买方E-MAIL：' = '买方邮箱 / Buyer E-mail:'
  '客户E-MAIL：' = '客户邮箱 / Customer E-mail:'
  '项目编号：' = '项目编号 / Ref. No.:'
  ...
  '注：' = ' / Notes:'
  "备`n注：" = "备注 / Notes:"
  "DCM1500FJ碟巢磨改性系统`n配置报价表" = "DCM1500FJ碟巢磨改性系统 / DCM1500FJ Disc-Nest Mill Modification System`n配置报价表 / Commercial Quotation"
  'DCM1500FJ碟巢磨改性系统' = 'DCM1500FJ碟巢磨改性系统 / DCM1500FJ Disc-Nest Mill Modification System'
  '配置报价表' = '配置报价表 / Commercial Quotation'
  ...
  '四、预干燥天然气直燃热风炉' = '四、预干燥天然气直燃热风炉 / IV. Pre-Drying Direct-Fired Gas Hot Air Furnace'
  ...
  '十二、改性天然气直燃热风炉' = '十二、改性天然气直燃热风炉 / XII. Direct-Fired Gas Hot Air Furnace for Modification'
  ...
  '十六、运费、安装调试费用，每套含以下内容：' = '十六、运费、安装调试费用 / XVI. Freight, Installation & Commissioning'
  '华通负责现场指导安装、调试，不含任何设备和操作钢架平台，供货由原料失重秤进料闸阀开始至收尘器出料旋转阀为止，其它可由买方根据华通设计施工图进行制作' = '华通负责现场指导安装、调试，不含任何设备和操作钢架平台，供货由原料失重秤进料闸阀开始至收尘器出料旋转阀为止，其它可由买方根据华通设计施工图进行制作 / Huatong shall provide on-site installation guidance and commissioning only. Equipment operating platforms are excluded. The supply scope shall commence at the inlet gate valve of the raw material loss-in-weight feeder and end at the discharge rotary valve of the dust collector. All remaining items may be fabricated by the Buyer in accordance with Huatong issued drawings.'
  ...
  '以上报价不含电缆和桥架,不含缓存料仓、水气管路(由买方提供),不含管道保温。' = '以上报价不含电缆和桥架,不含缓存料仓、水气管路(由买方提供),不含管道保温 / The above quotation excludes cables and cable trays, buffer silo, water and air piping to be provided by the Buyer, and pipe insulation.'
  ...
  '业主需要有机电操作维护能力人员，才能保持设备的最佳运转效率；' = '业主需要有机电操作维护能力人员，才能保持设备的最佳运转效率 / The Owner shall assign personnel with adequate electro-mechanical operation and maintenance capability to ensure optimum system performance.'
  '安装调试期间,操作维修人员必须参与教育训练,参与人员为操作、维修、现场主管,并设对口负责人员。' = '安装调试期间,操作维修人员必须参与教育训练,参与人员为操作、维修、现场主管,并设对口负责人员 / During installation and commissioning, operating staff, maintenance personnel, and site supervisors shall attend training, with designated counterparts assigned by the Owner.'
}

$formalTranslations = @{}
foreach ($key in $englishTranslations.Keys) {
  if ($formalOverrides.ContainsKey($key)) {
    $formalTranslations[$key] = $formalOverrides[$key]
  } else {
    $formalTranslations[$key] = "$key / $($englishTranslations[$key])"
  }
}
