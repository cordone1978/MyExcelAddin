...
const QUOTATION_ACCESS_PERMISSION = "quotation_access";
...
async function hasQuotationAccess(userId) {
  const id = Number(userId || 0);
  if (!id) return false;
  const [rows] = await pool.query(
    `SELECT 1 AS ok
     FROM app_user_permissions
     WHERE user_id = ? AND perm_code = ?
     LIMIT 1`,
    [id, QUOTATION_ACCESS_PERMISSION]
  );
...
async function requireAuth(req, res, next) {
  try {
    ...
    const allowQuotation = await hasQuotationAccess(session.userId);
    if (!allowQuotation) {
      res.status(403).json({
        success: false,
        error: SERVER_MESSAGES.authNoQuotationAccess || "当前账号无报价系统访问权限",
      });
      return;
    }
